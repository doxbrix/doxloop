...
import { approvedBatchLimits, assertBatchFits, batchLimits, defaultBatchLimits, hasPageLimit } from './batch-limits.js'
...
test('a new plan takes its batch from the documentation depth instead of a fixed small batch', () => {
  ...
  // Standard and comprehensive plans have no page cap; approved plans keep their original limits.
  expect(defaultBatchLimits('standard', true)).toEqual({ maxPages: 500, maxScreenshots: 300, maxMinutes: 45 })
  expect(defaultBatchLimits('comprehensive', true)).toEqual({ maxPages: 500, maxScreenshots: 300, maxMinutes: 120 })
  expect(defaultBatchLimits('custom', false)).toEqual({ maxPages: 500, maxScreenshots: 0, maxMinutes: 45 })
  expect(hasPageLimit(defaultBatchLimits('comprehensive', true))).toBe(false)
  expect(hasPageLimit(defaultBatchLimits('standard', true))).toBe(false)
  expect(hasPageLimit(defaultBatchLimits('starter', true))).toBe(true)
