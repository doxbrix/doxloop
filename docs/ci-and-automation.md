...
- **Advanced watch scope and budgets**: watched paths, ignored paths, the
  maximum agent minutes and runs per day, how many days verified evidence may
  age before it is re-verified, and whether expired verification warns or
  fails validation.
...
Scheduled monitoring requires a Git repository source. Private GitHub
repositories use the environment variable named by the source's `tokenEnv`
setting, `GITHUB_TOKEN` by default, which is read at runtime and never saved.
...
now** runs one cycle immediately and **Disable** removes the schedule while
keeping the saved settings for later.
